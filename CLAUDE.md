...
- **Комментарии в коде не писать** — ни пояснений, ни JSDoc-шапок, ни заметок про грабли,
  включая тесты. Оставляем только маркеры `// start .block` / `// end .block` и то, что
  было в файле раньше. Причины решений — в отчёт или `docs/`, не в код.
