...
- **Замер ≠ взгляд.** Перед словом «готово» открыть кадр каждого тронутого состояния (минимум
  390 и 1440). `getBoundingClientRect`/`getComputedStyle` отвечают только на заданный вопрос;
  выцветший фон, потерянный coverflow и чужое видео под правильным постером — всё это прошло
  зелёными цифрами и было поймано Pavel, а не мной (2026-08-17).
- **Перед выносом рабочего куска в общий компонент — снять baseline поведения** (эффект,
  размеры активного и соседних слайдов, порядок) и сверить после. Swiper, поднятый на скрытом
  контейнере, теряет 3D-эффект молча.
- **Плагин ведёт себя странно → его дока, до своих обходов.** Свои костыли вокруг чужого
  поведения ломают соседнее и потом всё равно удаляются.
- **Скриптовая правка файла — только с `assert` на совпадение паттерна.** Не совпавший
  `replace`/`sed` выглядит применённым: команда зелёная, файл прежний.
